/* < DTS2014121606223 caowei 201401217 begin */
/* add log */
...
/* < DTS2014122404177 caowei 201401224 begin */
#ifdef CONFIG_HUAWEI_HW_DEV_DCT
#include <linux/hw_dev_dec.h>
#endif
/* DTS2014122404177 caowei 201401224 end >*/
...
int cyttsp5_debug_mask = TP_INFO;
module_param_named(cyttsp5_debug_mask, cyttsp5_debug_mask, int, 0664);
/* DTS2014122209378 caowei 201401222 end >*/
...
static int cyttsp5_put_device_into_easy_wakeup_(struct cyttsp5_core_data *cd)
{
	...
	/* DTS2015011900593 caowei 20150119 end >*/
	...
	if(gesture_id == CYTTSP5_GESTURE_DOUBLE_CLICK && cd->double_tap_enabled) {
		for(i = 0; i < 2; i++) {
			if (cd->gest_pos[i].x < cd->dtz_x0 || cd->gest_pos[i].x > cd->dtz_x1||
				...
				tp_log_warning("%s %d:Click at:x0 = %d, y0 = %d\n", 
...
/* < DTS2015011503248  yuanbo 20150115 begin */
...
/* DTS2015011503248  yuanbo 20150115 end >*/
...
reset:
	...
	tp_log_info("%s: start to check i2c.\n", __func__);
	rc = cyttsp5_check_and_deassert_int(cd);

	/* < DTS2015012006679 caowei 20150120 begin */
	/* when reset fail 3 times, re-power the ic */
	...
	if ((0 == retry) && (rc < 0)) {
		...
		if (rc) {
			tp_log_err( "%s: Error on power on device r=%d\n", __func__, rc);
			goto exit;
		}
	...
	tp_log_info("%s: start up successful.\n",__func__);
exit:
	if (!rc)
...
static int cyttsp5_core_rt_suspend(struct device *dev)
{
	...
	tp_log_info("%s %d:Enter runtime suspend.\n", __func__, __LINE__);
...
static int cyttsp5_core_rt_resume(struct device *dev)
{
	...
	tp_log_info("%s %d:Enter runtime resume.\n", __func__, __LINE__);
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		tp_log_vdebug( "%s Device MAY wakeup\n", __func__);
...
/* < DTS2015012006679 caowei 20150120 begin */
	...
	SET_RUNTIME_PM_OPS(cyttsp5_core_rt_suspend, cyttsp5_core_rt_resume, NULL)
#elif defined (CONFIG_PM_SLEEP)
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_unlock(&cd->system_lock);
	tp_log_info("%s %d:easy_wakeup_gesture:%s\n", __func__, __LINE__, buf);
	/* DTS2014112404549 caowei 20141128 end > */
	return ret;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* < DTS2015011900593 caowei 20150119 begin */
	/* fix easy wakeup direction error */
...
/* < DTS2014120501789 caowei 201401205 begin */
/* < DTS2015012006679 caowei 20150120 begin */
#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_FB)
/* DTS2014120501789 caowei 201401205 end >*/
static void cyttsp5_pm_runtime_get(struct cyttsp5_core_data *cd)
{
	int i;

	tp_log_info( "%s:system count:%d,device count:%d,usage count:%d\n", __func__,
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	tp_log_info("%s %d:cyttsp5_core_probe start.\n", __func__, __LINE__);
	...
	/* DTS2014122209378 caowei 201401222 end >*/
	/* < DTS2015011503248  yuanbo 20150115 begin */
	cd->glove_suppert = cd->cpdata->glove_suppert;
	/* DTS2015011503248  yuanbo 20150115 end >*/
...
/* DTS2014122404177 caowei 201401224 end >*/
	...
	tp_log_info("%s %d:cyttsp5_core_probe success.\n", __func__, __LINE__);
...
/* DTS2014121606223 caowei 201401217 end >*/
